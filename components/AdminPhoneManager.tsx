...
export function AdminPhoneManager({ accessToken }: { accessToken: string }) {
  const [phones, setPhones] = useState<Phone[]>([]);
  const [offers, setOffers] = useState<PhonePrice[]>([]);
  const [selectedId, setSelectedId] = useState("");
  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState("");
  const [deleteState, deleteAction, deletePending] = useActionState(deletePhoneAction, initialState);
  const [offerState, offerAction, offerPending] = useActionState(saveOfferAction, initialState);
  const [deleteOfferState, deleteOffer, deleteOfferPending] = useActionState(deleteOfferAction, initialState);
